<script>
	...
	import { backHeader, changeHeader, errorMsg, user } from '../store/globalStore';
...
// Послать запрос на сервер - получить список классов
	...
	if($user.category == 'student') {
		let findGrade = grades.find(el => el.id == $user.classCode)
		if(findGrade) {
			gradeChoice(findGrade)
		} else {
			$errorMsg = {title: 'Нет данных', text: 'Данных для вашего класса не найдено'}
		}
